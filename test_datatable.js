	...
	if (participants_ass){
		for (var i = 0; i < participants_ass.length; i++){
			var
				...
				// GET FROM SAVED
				participant_user = getUserByID(g_act_parts, participant_id)
			;
			if (!participant_user){
...
//alert("33");
		var
			...
			assessment_name = getAssTitle(assessment)
		;
	...
	for (var i in assessors){
		if (i <= nRated - 1){
			...
			if (typeof(tmp) == 'object'){	// already
				...
				if (!assessor.img_id && g_impr_assrs){
					var user = getUserByID(g_impr_assrs, user_id)
					assessor.img_id = user.img_id;
					assessor.username = user.username;
				}
			} else {
				user_id = parseInt(tmp);
				assessor = getUserByID(g_impr_assrs, user_id);
			}
...
function getAssTitle(assessment){
