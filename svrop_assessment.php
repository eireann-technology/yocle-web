	...
	switch ($role){
		...
		case 'assessor':
			...
			switch ($method){
				case 'sur':
					/////////////////////////////////////////////////////////
					// SUR: NO INPUT IS NEEDED FOR SURVEY
					/////////////////////////////////////////////////////////
					
					$assr_asst_marks = 
					$part_asst_marks = 'Checked';
					
					// update assr_asst_marks & update part_asst_marks
					$update = ['$set' => 
						[	
							'assessment.assessments.$.assr_asst_marks.' . $assr_id . ',' . $part_id => $assr_asst_marks,
							'assessment.assessments.$.part_asst_marks.' . $part_id									=> $part_asst_marks,
							//'assessment.assessments.$.assr_asst_completed.' . $assr_id						=> $assr_asst_completed,
						]
					];
					if ($debug){
						print_json($update);
			...
			////////////////////////////////////////////
			// participants and num_of_marked
			////////////////////////////////////////////
			$num_of_participants = 0;
			$assr_asst_completed = 0;
			$options = [ 'projection' => ['_id' => 0, 'participants' => 1, 'assessment.assessments' => 1] ];
			$act_docs = databaseRead($database, $col_act, $filters_act, $options);
			if ($act_docs && sizeof($act_docs) > 0){
				$act_doc = json_decode(json_encode($act_docs[0]), true);

				// find participants
				$num_of_participants = sizeof($act_doc['participants']);

				// find num_of_marked
				$asst = $act_doc['assessment']['assessments'][$ass_index];
				if (isset($asst['assr_asst_marks'])){
					$marks_arr = $asst['assr_asst_marks'];
					$num_of_marked = 0;
					foreach ($marks_arr as $pair => $marks){
						$arr = explode(",", $pair);
						$assr_id2 = intval($arr[0]);
