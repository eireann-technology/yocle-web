	...
	switch ($data_type){
		...
		case 'user':
			...
			if ($user_id){
				...
				$result = databaseUpdate($database, 'users', ['user_id' => $user_id],
		...
		case 'activity':
			...
			if ($act_id){
				$result = databaseUpdate($database, 'activities', ['act_id' => $act_id],
		...
		case 'item':
			...
			if ($act_id){
				...
				$result = databaseUpdate($database, 'activities', ['act_id' => $act_id],
					[$action => ['assessment.assessments.'.($ass_id-1).'.items.'.($ass_item_id-1).'.media' => intval($media_id)] ]);
			}
			break;
	}
...
function getDocMedia($data_type, $ids){
	...
	if ($debug_svrop >= 1){
		wlog("getDocMedia data_type=$data_type ids=" . json_encode($ids));
	}
	$doc = 0;
	$path = 0;
	$criteria = 0;
	...
	// READ THE DOCUMENT
	//echo ("***$act_id $database $collection $path " . var_export($criteria, true)); exit();
	if ($path){
...
function uploadByResumable(){
	...
	if ($_SERVER['REQUEST_METHOD'] === 'GET'){
		...
		wlog($chunk_file . " not found");
		header("HTTP/1.0 404 Not Found");	// avoid console.error
	 }
	}
...
function uploadMedia(){
	...
	if ($media_id > 0){
		...
		if (!$data_type || !$ids){
			...
			$result = databaseDelete($database, 'media', ['media_id' => $media_id]);
			...
			if ($file_cat == ''){
				...
				$result = databaseInsert($database, 'media', $media);
...
function getMedia(){
	...
	$ids = getQS('ids');
	wlog("getMedia1: ".gettype($ids)." ".var_export($ids, true));
