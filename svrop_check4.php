...
/*
////////////////////////////////////////////////////////////////
// check point for singapore presentation
// random generator
////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////
// check point: 20170920
// for duplicate assignment 3 from old to new database
// update to database
/////////////////////////////////////////////////////////////////////
/*
define('ACT_ID', 132);
...
// read act
$acts = databaseRead('yolofolio', 'activities',
	[ 'act_id' => ACT_ID],
	[ 'projection' =>
		[
			'_id' => 0,
...
if (sizeof($acts) > 0){
	...
	forEach ($parts as $part_id){
		echo "copying $part_id...<br/>";

		// read old user
		$users1 = databaseRead('yolofolio20', 'users',
			[ 'user_id' => $part_id,
		...
		// read current user
		$users2 = databaseRead('yolofolio', 'users',
			[ 'user_id' => $part_id,
				'profile.activity.act_id' => ACT_ID,
			],
			[ 'projection' =>
		...
		if (sizeof($users1) > 0 && sizeof($users2) > 0){
			$user1 = $users1[0];
			$user2 = $users2[0];
			$uact1 = $user1->profile->activity[0];
			$uact2 = $user2->profile->activity[0];
			$uact2->assessments[3] = jsonclone($uact1->assessments[3]);

			//$asst2 = $user2->profile->activity[0]->assessments[3];
			//print_json($uact2);

			// update to database
	    $filters = ['user_id' => $part_id, 'profile.activity.act_id' => ACT_ID];
	  	$sets = ['profile.activity.$' => $uact2];
	    databaseUpdate('yolofolio', 'users', $filters, ['$set' => $sets]);
		}
	}
}
*/
...
/////////////////////////////////////////////////////////////////////
// check point: 20170921
// for copying the asst items from 118 to 132
// update to database
/////////////////////////////////////////////////////////////////////
/*
// read act
...
$acts1 = databaseRead('yolofolio', 'activities',
	[ 'act_id' => ACT_ID1],
	[ 'projection' =>
		[
			'_id' => 0,
...
$acts2 = databaseRead('yolofolio', 'activities',
	[ 'act_id' => ACT_ID2],
	[ 'projection' =>
		[
			'_id' => 0,
...
if (sizeof($acts1) > 0 && sizeof($acts2)){
	$act1 = $acts1[0];
	$act2 = $acts2[0];
	$parts1 = $act1->participants;
	$parts2 = $act2->participants;
	//print_json($act1);
	//print_json($act2);
	$part1_index = 1;
	forEach ($parts2 as $part2_id){
		$users2 = databaseRead('yolofolio', 'users',
			[ 'user_id' => $part2_id, 'profile.activity.act_id' => ACT_ID2],
			[ 'projection' =>
				[
					'_id' => 0,
					'user_id' => 1,
		...
		$asst2 = $users2[0]->profile->activity[0]->assessments[0];
		$items2 = $asst2->items;//->assessments->0->items);

		// loop until old user has an answer
		while (1)
		{
			if ($part1_index >= sizeof($parts1) - 1){
				$part1_index = 1;
			}
			$users1 = databaseRead('yolofolio', 'users',
				[ 'user_id' => $parts1[$part1_index++], 'profile.activity.act_id' => ACT_ID1],
				[ 'projection' =>
					[
						'_id' => 0,
						'user_id' => 1,
			...
			$asst1 = $users1[0]->profile->activity[0]->assessments[0];
			$items1 = $asst1->items;//->assessments->0->items);

			//echo "#" . $part1_index . ". " . $users1[0]->user_id . ' ';
			if ($items1[0]->answer != ''){
				//echo $items1[0]->answer . "<br/>";
				break;	// use this as answer
			} else {
				//echo " empty answer<br/>";
			}
		}

		forEach ($items2 as $item_index => $item2){
			$item2->answer = $items1[$item_index]->answer;
		}
		$items2[8]->answer = $users2[0]->username;
		$asst2->performed = $asst1->performed;

		//echo $users2[0]->user_id;
		//if (!isset($asst2->performed)) break;
		echo $part2_id;
		$uact = $users2[0]->profile->activity[0];
		print_json($uact);
		$filters = ['user_id' => $part2_id, 'profile.activity.act_id' => ACT_ID2];
		$sets = ['profile.activity.$' => $uact];
		databaseUpdate('yolofolio', 'users', $filters, ['$set' => $sets]);
		//break;
	}
}
...
///////////////////////////////////////////////////////////////////
// checkpoint: 20170926
// copy old data from old db to new db for act_id 118 HMT
///////////////////////////////////////////////////////////////////
/*
include 'svrop_calcgs.php';

define('ACT_ID', 118);
define('OLD_DB', 'yolofolio01');

$acts = databaseRead(OLD_DB, 'activities',
	[ 'act_id' => ACT_ID],
	[ 'projection' =>
		[
			'_id' => 0,
...
if (sizeof($acts) > 0){
	...
	// update it to the new database
	echo "working for act#".ACT_ID."...<br/>";
	//exit();
	
	databaseUpdate('yolofolio', 'activities', [ 'act_id' => ACT_ID], ['$set' => $act]);
	//exit();
	...
	forEach ($parts as $part_id){
		...
		echo "working for user#$part_id...<br/>";
		
		// read a participant
		$users = databaseRead(OLD_DB, 'users',
			['user_id' => $part_id, 'profile.activity.act_id' => ACT_ID],
			[ 'projection' =>
				[
					'_id' => 0,
					'profile.activity.$' => 1
...
///////////////////////////////////////////////////////////////////
// checkpoint: 20170926
// check for the answers for a particilar assignment
///////////////////////////////////////////////////////////////////

function checkPerformedAsst($act_id, $filter_ass_id, $check_item_media){	// 0=all
	...
	if (sizeof($acts) > 0){
		...
		// update it to the new database
		echo "<b>act#".$act_id." $act->title</b><br/>";
		...
		forEach ($parts as $part_id){
			...
			// read a participant
			$users = databaseRead('yolofolio', 'users',
			...
			$uact = $user->profile->activity[0];
			$showuser = 0;
	    forEach ($uact->assessments as $ass){
				if ($filter_ass_id != 0 && $filter_ass_id != $ass->ass_id){
					// filtered
				} else {
					$performed = isset($ass->performed) ? $ass->performed : '';
					if ($performed){
						if ($check_item_media){
							if (isset($ass->items[0]) && isset($ass->items[0]->media_id) && $ass->items[0]->media_id != 0){
								$media_id = $ass->items[0]->media_id;
							} else {
								continue;
							}
						}
						...
						if (!$showuser){
							echo "user#$part_id $user->username $user->email<br/>";
						}
						if ($check_item_media){
							echo "&nbsp;&nbsp;ass#$ass->ass_id: $performed media_id=$media_id<br/>";
							//print_json($user);
							//break;
						} else {
							echo "&nbsp;&nbsp;ass#$ass->ass_id: $performed<br/>";
						}
						$performed_count++;
					}
				}
		...
		echo "<br/>total: $performed_count<br/><br/><hr/>";
	}
}

checkPerformedAsst(118, 3, 1);	// HMT
checkPerformedAsst(119, 3, 1);	// Fanling
checkPerformedAsst(120, 3, 1);	// CLS
checkPerformedAsst(121, 3, 1);	// Aberdeen
checkPerformedAsst(130, 3, 1);	// SKU LMC
checkPerformedAsst(132, 3, 1);	// Cognitio
