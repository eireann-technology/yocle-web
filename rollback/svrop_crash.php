...
if (!$type){
	...
	switch ($type){
		...
		case 'make_friend':					make_friend();							break;
		case 'unfriend_user':				unfriend_user();						break;
		...
		if (!$success){
			...
			if ($username == ''){
				...
				if (!$existed){
					...
					wlog("previous secret_token=$secret_token");
					$user->email = $email;
					$user->pwd = $pwd;
					...
					$user->secret_token = $secret_token == '' ? create_guid() : $secret_token;
