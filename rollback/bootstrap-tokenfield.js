function getMatchedCSSRules2(element) {
      ...
      if (rule instanceof CSSMediaRule) {
        if (window.matchMedia(rule.conditionText).matches) {
          loopRules(rule.cssRules);
				...
				//console.info(element, $(element).matches);
				
				// crash on safari: inspecting by alantypoon 20161211
				if (!element){
					console.error('element is null');
					continue;
				} else if (!element.matches){
					console.error('element.matches is null');
					continue;
				}
				try {
					...
					//console.error('error', e); // ignorable
				}
      }
    }
  };
...
(function (factory) {
  ...
  * ============================== */
  ...
  var Tokenfield = function (element, options) {
    ...
    var elRules = (window && typeof window.getMatchedCSSRules === 'function') ? getMatchedCSSRules2( element ) : null
  ...
  , update: function (e) {
      ...
      if (this.$input.data('edit')) {
        ...
        this.$input.width( mirrorWidth )	// alantypoon 20160928: avoid the width is changed to pixel automatically
      }
      else {
        ...
        // so placeholder won't be cut off.
				//console.info(w);
        //isNaN(w) ? this.$input.width('100%') : this.$input.width(w); // alantypoon 20160928: avoid the width is changed to pixel automatically
