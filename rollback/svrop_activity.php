	...
	if (!$new_activity){
		...
		if ($act_id){
			...
			if (!$documents || !sizeof($documents)){
				...
				$new_assessments = $new_activity->assessment->assessments;
				
				// OLD ONES
				$old_activity = json_decode(json_encode($documents[0]), false);
				
				$old_assessments = $old_activity->assessment->assessments;
				
				foreach ($old_assessments as $index => $old_assessment){
					
					if (isset($new_assessments[$index])){
						$new_assessment = $new_assessments[$index];
						if (isset($old_assessment->assr_asst_completed)){
							$new_assessment->assr_asst_completed 	= $old_assessment->assr_asst_completed;
						}
						if (isset($old_assessment->part_asst_marks)){
							$new_assessment->part_asst_marks 			= $old_assessment->part_asst_marks;
						}
						if (isset($old_assessment->assr_asst_marks)){
							$new_assessment->assr_asst_marks			= $old_assessment->assr_asst_marks;
						}
						// for participation
						if (isset($old_assessment->selecteds)){
							$new_assessment->selecteds						= $old_assessment->selecteds;
						}
					}
				}				
				// remove the activity
...
function publishActivity(){
	...
	foreach ($all_users as $user_id => $v){
		...
		foreach ($new_uact->assessments as $index => $new_assessment){
			$index = intval($index);
			
			// PANALISTS
			$new_assessment->panelists->peer_assessors = [];
			
			// FOR EACH ITEM
			foreach ($new_assessment->items as $j => $item){
				$new_assessment->items[$j] = jsonclone($template_uact_ass_item);
				$new_assessment->items[$j]->ass_item_id = $j + 1;
			}
			...
			if ($old_uact && isset($old_uact->assessments)){
				...
				if (isset($old_uact->assessments[$index])){
					$old_assessment = $old_uact->assessments[$index];
					//print_json($old_assessment);
					
					// COPY OLD VALUES
					if (isset($old_assessment->saved)){
						$new_assessment->saved = $old_assessment->saved;
					}
					if (isset($old_assessment->performed)){
						$new_assessment->performed = $old_assessment->performed;
					}
					if (isset($old_assessment->marked)){
						$new_assessment->marked = $old_assessment->marked;
					}					
					if (isset($old_assessment->panelists) && isset($old_assessment->panelists->peer_assessors)){
						$new_assessment->panelists->peer_assessors = jsonclone($old_assessment->panelists->peer_assessors);
					}
					foreach ($new_assessment->items as $j => $item){
						if (isset($old_assessment->items) && isset($old_assessment->items[$j])){
							$new_assessment->items[$j] = jsonclone($old_assessment->items[$j]);
						} 
					}
				}
			}
			//echo "$new_assessment->method index=$index<br>"; print_json($new_assessment);
		}		
	...
	foreach ($all_users as $user_id => $v){	// all $v equal to 1 
		...
		if ($documents1 && sizeof($documents1) > 0){
			...
			if ($user_types != ''){
				...
				//print_json($user->profile->activity[0]->notified_invitation);
				if (
