...
function viewActImpr_assr1(jtbl, skills, assessees, markable){
	...
	for (var skill_name in skills){
		var
			...
			completeds = act_skill.act_assr_completeds
			completed = completeds && completeds[assr_id] ? parseInt(completeds[assr_id]) : 0
		;
		if (completed < 100){
...
function submitImpression(bSubmit){
	console.info('saveAssessment', 'submit='+bSubmit);
	var	
		act_id = g_saved_activity.act_id,
		assr_id = g_user_id
	;
