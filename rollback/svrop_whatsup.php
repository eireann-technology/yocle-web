		...
		if (is_array($whatsup->media_ids)){
			for ($j = 0; $j < sizeof($whatsup->media_ids); $j++){
				...
				if ($documents3 && sizeof($documents3)){
					...
					switch ($file_cat){
						case 'image':			$s .= '<img class="whatsup_image" src="./media/' . $file_name . '" style="width:'.$width.'"/>';		break;
						case 'video':			$s .= '<video class="whatsup_video" src="./media/' . $file_name . '" style="width:'.$width.'" controls></video>';	break;
					}
					$s .= '</td>'
						. '</tr>'
					;
