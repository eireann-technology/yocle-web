...
function createItem(){
	...
	var
		...
		,jtbl = $('#div_assessment_edit .my_datatable[dt_type=items]')
	;
	if (!assessment.items){
		assessment.items = [];
	}
...
function refreshEditItems(assessment){
	...
	for (var i = 0; i < items.length; i++){
		var item = items[i],
			item_id = i + 1
		;
		jtrs[i].after('<tr><td colspan="7">' + eval('getEditItem_' + method)(jtbl, dt, item_id, item) + '</td></tr>');
	}
