...
function createAssessment(method){
	...
	console.debug('createAssessment', method, assessment);
...
function addAssessment(method, assessment){
	...
	var jtbl = $('.my_datatable[dt_type=assessments]'),
			dt = jtbl.DataTable(),
			arr = [method_name, '<span class="ass_list_title">' + title + '</span><br/>' + g_percent_slider, '', g_but_edit_ass, g_but_trash_ass, ass_id, method]
	;
	jtbl.show();
	dt.row.add(arr).draw();
