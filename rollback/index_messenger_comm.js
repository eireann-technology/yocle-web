...
function resizeMessenger(){
	var h = g_nScreenH - 52 - $('#tbl_message_comm_input').outerHeight();
	//console.debug('resizeMessenger', h);
...
function openMessenger_user(user_id, user_name, msg_arr, users2){
	...
	var jdiv = $('#div_messenger_comm');
	jdiv.find('.bodyview_title').html(user_name);
...
function openMessenger_act(activity){
	...
	var hash = [].concat(activity.participants, [g_user_id, activity.coordinator_id]);
	g_recipients = hash2numArr(num2hashArr(hash));
...
function showUnreadMessages(){
	...
	console.debug('showUnreadMessages', unread);
	if (unread > 0){
