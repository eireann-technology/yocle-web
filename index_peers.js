...
function initPeers(){
	...
	$('.tbl_tabs_peers .inp_second_filter')
		.on('focus', function(e){
			this.select();
		})
		.on('keyup change', function(e){	//  change
			var searchkey = $('.inp_second_filter').val().toLowerCase();
			if (searchkey == g_last_searchkey){
				console.debug(e.type, searchkey, 'skipped');
				return;
			}
			console.info(e.type, searchkey);
...
function openPeers(){
	checkLoseInput(openPeers2);
}

/////////////////////////////////////////////////////////////////

function openPeers2(tab){
	console.info('openPeers2');
...
function showPeerTab(tab_index, searchkey){
	console.debug('showPeerTab', tab_index);
	...
	// select second tab
	selectSecondTab($('.tbl_tabs_peers'), tab_index, placeholder, 0);
