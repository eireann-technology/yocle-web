...
function initBasicLinked(jtbl, weights){

	// init slider and set event listener when any one is changed

	var sliders = jtbl.find('.basicLinked:not(.ui-slider)');	// not initialzed yet
	var nslider = sliders.length;
	if (!nslider){

		console.error('no sliders to init here');

	} else {

		console.debug('initBasicLinked', nslider, sliders);
		...
		// 2. initialize jquery ui sliders
		var jsliders = sliders.slider();
		if (nslider == 1){
			// set slider value
			var jslider = jtbl.find('.basicLinked');
			jslider.slider('value', 100);
			jslider.parent().next().text('100%');

		} else if (weights){
			// set slider values
			var i = 0;
			jtbl.find('.basicLinked').each(function(){
				$(this).slider("value", weights[i++]);
			});
		}
		// 3. show the value when sliding
		jsliders.on('slide slidestart slidestop slidechange', function(event, ui){ // slide
			var
				jslider = $(this),
				value = ui.value,
				jtbl = jslider.closest('table')
				//nslider = jtbl.find('.basicLinked').length
			;
			// always 100% if only one weight is there
			if (nslider == 1 && value != 100){
				value = 100;
				jslider.slider('value', value);
			}
			jslider.parent().next().text(value + '%');
		});

		// 4. set linkedsliders
		// destroy all linkedSliders
		//jtbl.find('.basicLinked').linkedSliders('destroy');

		// initialize the linkedSliders
		jtbl.find('.basicLinked').linkedSliders({policy: g_basicLinkPolicy});	// create sliders again

		// 5. add increment decrement
		jtbl.find('.but_left').click(function(){
			var jslider = $(this).closest('tr').find('.basiclinked');
			var
			...
			if (nslider == 1){
				...
				value2--; if (value2 < 0) value2 = 0;
			}
			if (value2 != value){
				jslider.slider('value', value2);
			}
		});

		// 6. add increment decrement
		jtbl.find('.but_right').click(function(){
			var jslider = $(this).closest('tr').find('.basiclinked');
			var
				value = jslider.slider('value'),
				value2 = value
			...
			if (nslider == 1){
				...
				value2++; if (value2 > 100) value2 = 100;
			}
			if (value2 != value){
				jslider.slider('value', value2);
			}
...
function getEvenWeightings(n){
	var av = Math.round(100 / n), total = 100;
	var arr = [];
	for (var i = 0; i < n; i++){
		if (i < n - 1){
			arr[i] = av;
