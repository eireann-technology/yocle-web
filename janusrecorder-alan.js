...
var server = "https://yocle.net:8089/janus";
...
function initJanus(){
	...
	if (!Janus.isWebrtcSupported()) {
		bootbox_alert("No WebRTC support... ");
		return;
	}
	// Create session
	janus = new Janus(
				...
				janus.attach(
					{
						...
						error: function(error) {
							...
							bootbox_alert("  -- Error attaching plugin... " + error);
						},
								...
								if(result["status"] !== undefined && result["status"] !== null) {
									...
									if(event === 'preparing') {
										...
										recordplay.createAnswer(
											{
												...
												error: function(error) {
													...
													bootbox_alert("WebRTC error... " + JSON.stringify(error));
												}
											});
										//if(result["warning"])
										//	bootbox_alert(result["warning"]);
										...
										if(recordingId !== null && recordingId !== undefined) {
											...
											//bootbox_alert("Recording completed! Check the list of recordings to replay it.");
										}
...
function updateRecsList() {
	...
	recordplay.send({"message": body, success: function(result) {
		...
		if(result === null || result === undefined) {
			bootbox_alert("Got no response to our query for available recordings");
			return;
		}
		if(result["list"] !== undefined && result["list"] !== null) {
			...
			$('#recslist a').unbind('click').click(function() {
				...
				//$('#play').removeAttr('disabled').click(startPlayout);
				var video = selectedRecordingInfo.indexOf('video') == 0;
				startPlayout2(selectedRecording, video);
				return false;
			});
		}
	}});
...
function startRecording(video){
	...
	//bootbox.prompt("Insert a name for the recording (e.g., John Smith says hello)", function(result) 
	{
		...
		myname = video ? "video recording" : "audio recording";
		if (video){
			$('#videobox').show();
		} else {
			$('#videobox').hide();
		...
		recordplay.createOffer(
			{
				...
				error: function(error) {
					Janus.error("WebRTC error...", error);
					bootbox_alert("WebRTC error... " + error);
					recordplay.hangup();
				}
			});
	};
...
///////////////////////////////////////////////////////////////
// by alantypoon 20180209
///////////////////////////////////////////////////////////////

function startPlayout2(selectedRecording, video){
	console.log('startPlayout2', selectedRecording);
...
function bootbox_alert(s){
	bootbox.alert(s);
}

////////////////////////////////////////////////////////////////

function enable_record(enable){
	if (enable){
		$('#record_audio').removeAttr('disabled').unbind().click(function(){startRecording(0)});
		$('#record_video').removeAttr('disabled').unbind().click(function(){startRecording(1)});
	} else {
		$('.record_btn').unbind('click').attr('disabled', true);		
	}
}
