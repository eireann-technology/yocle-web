...
function initAll(){
	...
	$('.toggle_type').on('toggle', function(e, active) {
		$('#td_activity_type')
			//.text('('+getLangStr(active?'.text_oclx_full':'.text_yolox_full')+')')
			.addClass(active?'text_oclx_full':'text_yolox_full')
	...
	$('.toggle_assess').on('toggle', function(e, active) {
		...
		//$('#td_assess').text(active?'(Assessment is enabled for this activity)':'(Assessment is disabled for this activity)')
		$('#td_assess')
			.addClass(active?'text_enable_assess':'text_disable_assess')
	...
	var i = 0, gsarr = $('#tbl_gsstatus .gs_rating2');
	[4, 3.5, 3.5, 4.5, 4, 4].forEach(function(rating){
		var jstar = gsarr.eq(i++);
		jstar.starRating('setRating', rating);
	});
	var i = 0, commentarr = $('#tbl_gsstatus .gs_text_box');
	['Good work! Wilson has outstanding team-building skills and builds a team-oriented attitude among all teammates. (Prof. David Japser)', 'Well done! Marianna has an extraordinary ability to turn a group into a team and pulls people together into a cooperative and successful team. (Ian smith)'].forEach(function(s){
...
function openDropmenu(obj, menu, event){
	...
	switch (menu){
		...
		case 'searchact':
			...
			$('#dropmenu_searchact li:last-child').show();
			break;
...
//////////////////////////////////////////////////////////
//var g_person_name = '';
function openAssessment(obj){
	var activity = g_activity_arr[g_activity_index],
			task = activity.tasks[g_task_index],
			jbut = $(obj),
			person_name = jbut.parent().parent().find('td:nth-child(2)').html()
	;
	//g_person_name = person_name;
	//console.debug(jobj[0].outerHTML);
	...
	$('#dialog_eva')
		.attr('title', getLangStr('dialog_eva'))
		.dialog({
			modal: true,
			autoOpen: true,
			resizable: false,
...
function setGauge1(jgauge1, value){
	jgauge1.parent().find('.preview-textfield').html(value);
	var data = jgauge1.data();
	if (data){
		// delete otherwise the pointer or color will not be gone
		// and create the gauge again
		var opts = data.gauge.options;
		delete data.gauge
		data.gauge = new Gauge(jgauge1[0]).setOptions(opts);
	} //else 
...
function addSkill(){
	var skill = $('#tbl_gskills input.custom-combobox-input').val();
	if (skill != ''){
		// check if it already exists
		var bFound = 0;
		$('#tbl_gskills2 .datatable tbody tr td:first-child').each(function(){
	...
	$('.datatable_printable').DataTable({
		...
		buttons: [{
			...
			customize: function ( win ) {
				...
				jtbody.find('> tr > td:nth-child(2)');		// name
				var td_gs = jtbody.find('> tr > td:nth-child(3)');		// generic skills
				td_gs.find('td:nth-child(1)').css('width', 100);
				td_gs.find('td:nth-child(2)').css('width', 100);
...
function openGsStatus(){
	var title = getLangStr('dialog_gsstatus'),
			activity = g_activity_arr[g_activity_index],
			task = activity?activity.tasks[g_task_index]:'';
	$('#gsstatus_title').text(activity?activity.title:'');
	$('#gsstatus_date').text((activity?activity.start:'') + ' - ' + (activity?activity.end:''));
	$('#gsstatus_task').text(task?task.title:'');
	$('.buttons-print').hide();
