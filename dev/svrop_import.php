...
function createUser($email, $pwd, $username, $gender, $birthday, $confirmed_email, $secret_token){
	...
	$user->next_send_confirm = 0;//getNextSendConfirm();
...
function importUsers($users){
	...
	if (gettype($users) == 'array'){
		...
		foreach ($users as $user){
			...
			$user_id = 0;
			if (sizeof($arr) == 1){
				// only email, general default fields
				$email = $user;
				$username = explode('@', $user)[0];
				$user = "$email,$username,M,2018-04-20";
				//echo $user . "<br/>";
				$arr = explode(',', $user);
			}
			if (sizeof($arr) != 4){
				echo "invalid input: line $line, " . sizeof($arr);
				print_json($arr);
			} else {
				$email = trim($arr[0]);
		...
		//print_json($user_ids);
		echo "<br><br>" . implode(', ', $user_ids);

		echo "<br><br>No. of users=" . sizeof($user_ids);
...
function importUsersFromFile(){
	...
	$path = getcwd() . DIRECTORY_SEPARATOR . '/_importusers/' . $file_name;
	if (file_exists($path)){
