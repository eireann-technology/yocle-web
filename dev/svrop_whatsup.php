...
function read_whatsup(){
	global $database, $error, $type, $email, $pwd, $error, $output, $secret, $template_user, $user_id, $g_media_url, $test_qs;
		...
		if (is_array($whatsup->media_ids)){
			for ($j = 0; $j < sizeof($whatsup->media_ids); $j++){
				...
				if ($documents3 && sizeof($documents3)){
					...
					switch ($file_cat){
						case 'image':			$s .= '<img class="whatsup_image" src="' . $g_media_url . $file_name . '" style="width:'.$width.'px"/>';		break;
						case 'video':			$s .= '<video class="whatsup_video" src="' . $g_media_url . $file_name . '" style="width:'.$width.'px" controls></video>';	break;
					}
					$s .= '</td>'
						. '</tr>'
					;
