...
function createWhatsup(media_id_arr){
	console.log('createWhatsup', media_id_arr);

	var jdiv = $('#div_whatsup');
	jdiv.find('.div_whatsup_output').hide();
...
function openWhatsup(whatsup_id){
	...
	// output
	var jscroll = g_jscroll_whatsup = $('.div_whatsup_output');
	...
	jscroll
		.jscroll({
			...
			callback: function(a){
				...
				// tooltip
				var jdiv = jscroll.find('.whatsup_tooltip');
				jdiv.tooltip({html: true});

				// autogrow
				jscroll.find('.whatsup_mycomments').autoGrow().unbind().keypress(function(e){
				...
				// like action
				jscroll.find('.whatsup_img_likes').unbind().click(function(){
					//console.debug('like');
