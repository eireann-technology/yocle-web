...
$.widget( "a.uploader", {
	...
	_create: function() {
		...
		if (el.prop("tagName") != 'INPUT' || el.attr('type').toUpperCase() != 'FILE'){
			...
			var self = this;
			if (!g_resumable){
				g_resumable = new Resumable({
					target: self.options.target,
					query: self.options.query,
					testChunks: 0,	// overwrite every time
				});
			}
			
			g_resumable.assignBrowse(el[0]);
			
			// EVENTS
			g_resumable.on('fileAdded', function(file){
				...
				if (file_cat == ''){
					...
					switch (file_cat){
						case 'image': self._previewImage(blob_url, file_id, 0, self._addProgressBar, file); break;
						case 'video': self._previewVideo(blob_url, file_id, 0, self._addProgressBar, file); break;
					}
				}
			});
			...
			g_resumable.on('fileSuccess', function(file, message){
				var jdiv = file.jdiv,
					jsvg = jdiv.find('svg'),
				...
				// remove file_id
				var file = g_resumable.getFromUniqueIdentifier(file_id);
				g_resumable.removeFile(file);
				jdiv.attr('file_id', 0);
			...
			g_resumable.on('fileProgress', function(file){
				var progress = file.progress();
				file.pbar.set(progress);	// (0-1, 1=100%)
			});
			g_resumable.on('fileError', function(file, message){
				console.error('fileError', file);
			});
			g_resumable.on('complete', function(){
	...
	// previewImage
	////////////////////////////////////////////////////////////
	_previewImage: function(url, file_id, media_id, onLoad, file){
		...
		jobj.load(function(){
			...
			self._setHover(jobj);
			onLoad && onLoad.call(self, file, jdiv, tn.w, tn.h);
		})
		.attr('src', url);
	},
	...
	// previewVideo
	////////////////////////////////////////////////////////////
	_previewVideo: function(url, file_id, media_id, onLoad, file){
	...
	////////////////////////////////////////////////////////////
	// addProgressBar
	_addProgressBar: function(file, jdiv, w, h){
		console.debug('_addProgressBar', file.uniqueIdentifier);
		////////////////////////////////////////////////////////////
		// ADD PROGRESS
		// https://kimmobrunfeldt.github.io/progressbar.js/
		////////////////////////////////////////////////////////////
