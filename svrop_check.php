...
forEach ($users as $user1){
  ...
  $hasmedia = 0;
    ...
    // special rule
    if (!$dummy && $act_id != DUMMY_ACT_ID) $uact_old = $uact;
    if ($act_id == DUMMY_ACT_ID) $uact_new = $uact;
  ...
  // special
  if ($uact_old && $uact_new){

    if ($hasmedia){
      array_push($parts, $user_id);
    }
/*
    uact->assessments->items = [
  		{
  				"ass_item_id" : 1,
  				"part_item_marks" : 80,
